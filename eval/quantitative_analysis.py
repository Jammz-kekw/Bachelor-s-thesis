...
if __name__ == '__main__':
    ...
    he_l = np.load('4x_mean_new//HE_L_gen_bha.npy')
    he_a = np.load('4x_mean_new//HE_A_gen_bha.npy')
    he_b = np.load('4x_mean_new//HE_L_gen_bha.npy')

    he_lab = get_mean(he_l, he_a, he_b)

    he_l_norm = np.load('4x_mean_new//HE_L_norm_bha.npy')
    he_a_norm = np.load('4x_mean_new//HE_A_norm_bha.npy')
    he_b_norm = np.load('4x_mean_new//HE_B_norm_bha.npy')

    he_lab_norm = get_mean(he_l_norm, he_a_norm, he_b_norm)

    ihc_l = np.load('4x_mean_new//IHC_L_gen_bha.npy')
    ihc_a = np.load('4x_mean_new//IHC_A_gen_bha.npy')
    ihc_b = np.load('4x_mean_new//IHC_B_gen_bha.npy')

    ihc_lab = get_mean(ihc_l, ihc_a, ihc_b)

    ihc_l_norm = np.load('4x_mean_new//IHC_L_norm_bha.npy')
    ihc_a_norm = np.load('4x_mean_new//IHC_A_norm_bha.npy')
    ihc_b_norm = np.load('4x_mean_new//IHC_B_norm_bha.npy')
